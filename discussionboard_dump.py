...
def discussion_content(class_id,token):
    ...
    for i in r:
        f = csv.writer(open("topic_info.csv",'a'))
    ...
    print("Extracted all info starting to dispatch correct info to each discussion csv")
    for q in topic_list:
        f = csv.writer(open(q + '.csv', 'a'))
        f.writerow(['comment/reply', 'id', 'parent_id', 'created_at', 'updated_at', 'message', 'user_id']) #These are the headers for the csv
    ...
    for q in topic_list:
        ...
        r = r.json()
        print ("start writing data to "+ q +" file")
        ...
        print("finished writing all data. " +q+".csv file ready to be opened")
